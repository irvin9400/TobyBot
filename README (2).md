...
- `GAME_MOD_LOG_CHANNEL_ID` — channel for in-game ban/kick/warn events
- `GAME_ACTION_LOG_CHANNEL_ID` — channel for in-game unban/jail/unjail/freeze/unfreeze events
...
3. In the Variables tab, add every key from `.env` (`DISCORD_TOKEN`, `CLIENT_ID`, `GUILD_ID`, `MOD_LOG_CHANNEL_ID`, `GAME_MOD_LOG_CHANNEL_ID`, `GAME_ACTION_LOG_CHANNEL_ID`, `MOD_ROLE_IDS`, `WEBHOOK_SECRET`) with your real values. You don't need to set `PORT` — Railway assigns it automatically and the bot reads it.
...
Only these action names are accepted and logged: `ban`, `kick`, `warn` (posted to `GAME_MOD_LOG_CHANNEL_ID`), and `unban`, `jail`, `unjail`, `freeze`, `unfreeze` (posted to `GAME_ACTION_LOG_CHANNEL_ID`). Anything else — including things like `lock entrance` / `unlock entrance` — is rejected with a 400 response and never posted to Discord. To change what's logged, or which group an action belongs to, edit `MODERATION_ACTIONS` and `IN_GAME_ACTIONS` in `webhook/server.js`.
